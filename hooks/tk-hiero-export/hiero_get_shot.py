...
class HieroGetShot(Hook):
	...
	def get_episode(self, data=None, hiero_sequence=None):
		...
		# parse episode name from seq name
		nuke_studio_episode = hiero_sequence.name().split('_')[0]
		if not nuke_studio_episode:
			raise StandardError("No episode could be parsed from the sequence: %s" % hiero_sequence.name())
		...
		if nuke_studio_episode in data["epi_cache"]:
			return data["epi_cache"][nuke_studio_episode]
		...
		data["epi_cache"][nuke_studio_episode] = episode
	...
	def get_shot_parent(self, hiero_sequence, data, **kwargs):
		...
		if len(parents) > 1:
			...
			raise StandardError( "Multiple %s entities named '%s' found" % (par_entity_type, hiero_sequence.name()) )
		if len(parents) == 0:
